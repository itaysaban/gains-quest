...
- **Epic 3, Stories 3.1/3.2/3.4/3.5: DONE, live-Postgres verified 2026-08-15 — built autonomously while the user was away, per their explicit go-ahead ("auto accepted... until we get to story 3.3").** Story 3.3 (the hard XP/Level cutover) is deliberately NOT started — held for the user's return, per their own instruction, since it's the irreversible step. What shipped:
  ...
  - **Design/estimation decisions made without the user present — flag for confirmation on return:**
    - **4 of 10 badges' point values are estimated**, not sourced from the PRD or the Figma (neither had them): Progressive Overload 1000, Architect 500, Unbroken 1500, Well Rounded 750. The other 6 are confirmed exact (First Workout 500, Speed Demon 1000, Iron Will 750, Century Club 2000, Social Butterfly 750, Tonnage 2500 — from the Figma screenshot and/or epics.md's own text).
    - **Social Butterfly's description text doesn't match the Figma.** The Figma shows "Give 50 High-Fives"; epics.md's already-decided (and already-tested) criteria is `friend_count` (5 friends, stubbed false until M4's friends system ships). Kept the `friend_count` mechanic — it has a real, tested spec — and wrote description copy ("Make 5 training friends") honest about what's actually gated, rather than shipping Figma's copy for a "high-five" feature that doesn't exist anywhere in the app. If "Give 50 High-Fives" is actually preferred as the real mechanic, that's a scope decision for the user, not something to guess at.
  ...
  - Nothing pushed to GitHub or deployed to any Supabase project (test or production) — all local, committed, and ready for review.
- **Epic 3, Story 3.3 (the hard cutover): NOT STARTED, deliberately held.** Swaps Home/Achievements to real GP display and drops the old XP/Level system for good — the one irreversible step in Epic 3. Waiting for the user to be present before starting this, per their own explicit instruction when authorizing the rest of Epic 3 to proceed without them.
