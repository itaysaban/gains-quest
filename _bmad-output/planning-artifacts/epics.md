...
- **M4 (Social), Story 1 — Friends: CODE DONE 2026-08-18, live-Postgres verified, NOT YET DEPLOYED to production.** Per the PRD's release plan (§10), M4 is next after M3. Rather than blocking all of M4 on the still-open leaderboard-tier-size question (§11), started with the one piece that's unambiguous per the PRD and is a real prerequisite for both the Friends leaderboard and the social feed. `20260818000001_friends.sql`:
  ...
  - **Still explicitly NOT started:** both leaderboards (global tiers + friends board — blocked on §11's open tier-size question, "is 100 the right cohort, or should it scale with the user base?"), seasons, the social feed, reactions. Starting those needs either the user's call on tier size or a decision to ship a default (the design handoff's own example screen used 100 without flagging it) — a product call, not made unilaterally here.
