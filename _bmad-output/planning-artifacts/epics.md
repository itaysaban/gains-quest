...
- **CRITICAL — P0 regression found by live user testing immediately after the above deploy: "did a leg exercise, did it again, didn't show the last one."** The always-visible last-time row (PRD §6.1.3: "This is the feature the product lives or dies on") reads `exercise_current_best.last_session_exercise_id` (`hooks/useLastSessionSets.ts`). `fn_complete_session` is supposed to stamp that pointer on every completion — that block existed from `20260814000005_exercise_current_best.sql` onward but was **silently dropped** when `fn_complete_session` was rewritten in `20260817000001_drop_legacy_xp_system.sql` (stripping old XP bookkeeping) and stayed missing through every redefinition since, including `20260820000001_social_feed.sql` and this session's own `20260824000001` cutover fix — all three carried the gap forward. No scenario test ever asserted on this specific column across the entire session, so nothing caught it until the user hit it live.
  ...
  - **Deployed to production 2026-08-30, user confirmed the last-time row is now working correctly.**
- **M5 (Polish) — Privacy settings, Notifications audit, and Empty-states sweep: CODE DONE 2026-08-30.** All three remaining M5 areas (Onboarding and Pause Mode were already done). User asked for all three at once.
  - **Privacy settings.** PRD §9's only concrete privacy requirement: "Leaderboard participation can be turned off entirely without losing points or badges." `20260826000001_privacy_leaderboard_opt_out.sql` adds `profiles.leaderboard_opt_out boolean not null default false` (client-updatable directly via the existing `profiles_update_own` RLS policy, same pattern as every other simple profile setting) and redefines `fn_leaderboard` (last defined `20260819000001_leaderboard.sql`, never touched since) to exclude an opted-out user from **both** scopes' ranking pool entirely — not just hidden from the output with a rank gap left behind, everyone else's rank closes up around them — while leaving `point_ledger`/`personal_records`/`user_badges` completely untouched (a pure ranking-visibility toggle) and still letting the opted-out user view either board themselves (their own row just never appears). `fn_lifetime_stats`' `season_rank` deliberately left alone — that's a private stat shown only to the user themselves, not something visible to others the way the ranked boards are, matching the PRD's own lifetime-vs-ranked-board distinction (§6.2). Client: `app/(tabs)/settings/privacy.tsx`, a single toggle, linked from Settings — no dedicated Jest screen test (matches the existing precedent of `units.tsx`/`weekly-goal.tsx`, similarly-trivial single-mutation screens with no dedicated coverage; the real logic is server-side and thoroughly tested there). **Verification:** 10 new live-Postgres scenario tests (`test-epic9.js`) — including one genuine RLS check (`set role authenticated` + `set_config`, not the harness's own BYPASSRLS superuser connection) proving a user can set their own flag for real. One test-pollution fix during writing: asserting an exact absolute rank in this shared, long-lived scratch DB is unreliable (other epics' dominant-GP test users can outrank a fresh test's "control" user) — switched to asserting the returned ranks are contiguous 1..N with no gap, which is pollution-independent and still proves the real claim (excluded from the pool, not just hidden).
  - **Notifications audit — no code changes needed.** Read `lib/notifications/streakReminder.ts`, `hooks/useStreakReminder.ts`, `app/(tabs)/settings/notifications.tsx`, and the existing `lib/notifications/__tests__/streakReminder.test.ts` (10 tests, already passing) in full before touching anything. All 3 of PRD §6.3's requirements are already correctly implemented: (1) one reminder per day max, at the user's historically-most-common training hour (`pickReminderHour`), via a single fixed notification identifier that's always cancelled-then-rescheduled, never stacked; (2) the "at risk" nudge only fires when rest allowance **and** banked freezes are both exhausted (`evaluateStreakReminder`'s `exhausted` check), a plain reminder otherwise; (3) Pause Mode suppresses it entirely (`isPausedOn`), and the copy is genuinely guilt-free ("whenever you're ready", not "you're losing your streak!"). The PRD's "logged an injury" sub-clause has no separate mechanism anywhere in this codebase — Pause Mode's own settings copy explicitly frames itself as the injury/illness/travel remedy ("Dealing with injury, illness, or travel? Pause Mode holds your streak exactly where it is"), so that clause is satisfied by the same check already in place, not a gap. Confirmed `useStreakReminder()` is actually mounted (Home) and re-runs on every dependency change, including after a session completes (via `['streak']`'s existing invalidation triggering a refetch that re-fires the effect) — genuinely wired end-to-end, not just correct in isolation.
  - **Empty-states sweep — no gaps found.** Swept every screen under `app/` for a legitimately-reachable zero-data case rendering as a blank view instead of a real `EmptyState`. Already correctly handled everywhere it matters: Leaderboard, Friends (a lighter inline-text pattern, not the full component, but a real message either way — left as-is per "don't touch what already works"), Social Feed, the routine list/detail screens, session/active, and exercise progress charts (`StrengthLineChart` has its own built-in "log a few sets to see your progression here" fallback, covering Body Measurements' chart too). Checked and ruled out as non-issues: Achievement Hall (never actually empty — every badge in the catalogue is always shown, locked or not, by PRD design intent, §6.4); the Progress tab's "Exercise History" list (never actually empty in practice — it lists the full ~200-exercise seeded library, not literal user history, so a brand-new user still sees a full list); exercise/session detail routes falling back to a loading spinner on a bad ID (an invalid-deep-link edge case, not a zero-data-for-a-new-user case, and out of this sweep's scope). No client or server changes from this part.
  - **Verification (all 3 parts):** typecheck clean, full Jest suite green at 211 (unchanged from before this batch — Privacy's 10 new tests are live-Postgres scenario tests, `test-epic9.js`, not Jest; Notifications and Empty-states needed no code changes at all). One single-run flake hit mid-verification: `CompleteDiscardSession.e2e.test.tsx` failed once on a `waitFor` in a full-suite run, untouched by anything in this batch — reran it isolated (clean) and reran the full suite twice more (clean both times), confirming an occasional cross-test flake consistent with the "worker process failed to exit gracefully... tests leaking" warning already a known, harmless characteristic of this environment all session, not a real regression. Committed (`ee2fe87`) and pushed to `origin/master`.
  - **Deployed to production 2026-08-30, user confirmed working as intended.** All of M5 (Onboarding, Pause Mode, Privacy, Notifications, Empty-states) is now shipped.
