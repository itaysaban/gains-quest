-- M3 Epic 3 Story 3.3: the hard cutover. GainPoints (point_ledger) has been live, verified, and
-- displayed in the Achievement Hall since earlier in M3 — this migration retires the XP/Level system
-- it was always meant to replace. Irreversible: once xp_events/user_levels/level_thresholds are
-- dropped, the old per-user XP/level numbers are gone for good (per epics.md's repeated warning).
--
-- Order: strip every write path first (so nothing inserts into a table we're about to drop), then
-- drop the now-unreferenced trigger, functions, tables, enum, and the workout_sessions.xp_earned
-- column (dead the moment nothing updates it anymore).

-- fn_process_logged_set (last defined 20260814000005_exercise_current_best.sql): drop both xp_events
-- inserts (warmup flat-XP, working-set fn_calculate_set_xp). Everything else — PR detection, the
-- exercise_current_best cache — is unchanged.
...
-- fn_check_badges (last defined 20260815000003_badge_criteria_v1.sql): drop the xp_events insert,
-- keep the point_ledger 'achievement' insert.
...
-- fn_complete_session (last defined 20260814000011_streak_rest_allowance_freezes.sql): drop the
-- xp_before/xp_after/level_before/level_after bookkeeping, the session-completion xp_events insert,
-- and the xp_earned/leveled_up/new_level return fields — points_earned (from
-- fn_award_points_for_session) is now the only "how much did I earn" field the client reads.
...
-- handle_new_user (originally 20260811000008_functions_triggers.sql): stop bootstrapping a
-- user_levels row for new signups.
...
-- Nothing writes to xp_events anymore (all three insert sites above have been redefined) — safe to
-- drop the trigger that recomputed user_levels off it, the recompute function itself, the per-set XP
-- calculator, the three legacy tables, the now-orphaned enum type, and the workout_sessions column
-- that only that trigger chain ever kept current.
drop trigger if exists xp_events_update_level on public.xp_events;
